...
public class PetStoreManagerServiceImpl implements PetStoreManagerService {
    ...
    public void retrievePassword(RetrievePasswordDTO retrievePasswordDTO) {
        PetStoreManager petStoreManager = petStoreManagerMapper.selectByTel(retrievePasswordDTO.getTel());
        if (petStoreManager == null) {
            throw new CustomException("手机号不存在");
        }
        //TODO 校验验证码
        petStoreManager.setPassword(retrievePasswordDTO.getPassword());
        petStoreManagerMapper.updateById(petStoreManager);
    }
