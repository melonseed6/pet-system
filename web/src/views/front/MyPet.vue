      ...
      <el-row :gutter="20">
        <el-col :span="6" v-for="(item,index) in listData">
          <el-card shadow="hover" :style="index>3 ? { marginTop: '10px' } : null">
            ...
            <template #footer>
              <a link href="javascript:;" @click="feed(item)">上门喂养</a>
              <a link href="javascript:;" @click="fosterCare(item)">临时寄养</a>
              <a link href="javascript:;" @click="edit(index, item)">编辑</a>
              <a link href="javascript:;" @click="deleteOne(index, item)">删除</a>
            </template>
          </el-card>
